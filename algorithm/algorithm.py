...
class Algorithm(torch.nn.Module):
    ...
    #         'prec': Precision().to(self.rank),
    #         'rec': Recall().to(self.rank),
    #         'loss': MeanMetric().to(self.rank)
    #     }
    #     if self.rank == 0:
    #         wandb.define_metric("train/acc", summary="max")
    ...
    def _init_metrics(self):
        num_classes = 5  # Ganti nilai ini sesuai dengan jumlah kelas Anda
        train_metrics = {
            'acc': Accuracy(task="MULTICLASS", num_classes=num_classes).to(self.rank),
            'prec': Precision().to(self.rank),
            'rec': Recall().to(self.rank),
            'loss': MeanMetric().to(self.rank)
        }
